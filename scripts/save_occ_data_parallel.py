...
def sample_occ(mesh_pose_list_path, num_point, uniform):
    meshpath_list = np.load(mesh_pose_list_path, allow_pickle=True)['ml']
    scale_list = np.load(mesh_pose_list_path, allow_pickle=True)['sl']
    pose_list = np.load(mesh_pose_list_path, allow_pickle=True)['pl']
    scene, mesh_list = get_scene_from_mesh_pose_list(meshpath_list, scale_list, pose_list, return_list=True)
...
def main(args):
    ...
    if args.num_proc > 1:
        ...
        for f in tqdm(mesh_list_files, total=len(mesh_list_files)):
            save_occ(f, args)
