...
def get_mesh_pose_list_from_world(world, object_set, exclude_plane=True):
    ...
    return meshpath_list, scale_list, pose_list


def get_scene_from_mesh_pose_list(meshpath_list, scale_list, pose_list, scene_as_mesh=True, return_list=False):
    # create scene from meshes
    scene = trimesh.Scene()
    mesh_list = []
    for i in range(len(meshpath_list)):
        mesh_path = meshpath_list[i]
        scale = scale_list[i]
        pose = pose_list[i]
