...
class TSDFVolume(object):
    ...
    # def get_grid(self):
    #     # TODO(mbreyer) very slow (~35 ms / 50 ms of the whole pipeline) giga的这个函数会报错段错误 (核心已转储)
    ...
    def get_tsdf(self):
        volume_tsdf = self._volume.extract_volume_tsdf()
        tsdf_real = np.asarray(volume_tsdf)[:, 0].reshape(self.resolution, self.resolution, self.resolution)
        tsdf_mask = np.asarray(volume_tsdf)[:, 1].reshape(self.resolution, self.resolution, self.resolution)
        tsdf_real[tsdf_mask == 0] = -1
        return tsdf_real
