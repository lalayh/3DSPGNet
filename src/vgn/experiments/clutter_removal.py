...
def run(
    ...
    for _ in tqdm.tqdm(range(num_rounds), disable=silence):
        ...
        while sim.num_objects > 0 and consecutive_failures < MAX_CONSECUTIVE_FAILURES:
            ...
            # scan the scene
            tsdf, pc, timings["integration"], intrinsic, extrinsic, rgb_imgs, depth_imgs = sim.acquire_tsdf(n=n, N=N, resolution=40)
            ...
            # plan grasps
            if visualize:
                meshpath_list, scale_list, pose_list = get_mesh_pose_list_from_world(sim.world, object_set)
                scene_mesh = get_scene_from_mesh_pose_list(meshpath_list, scale_list, pose_list)
                if model_type == 'former3d':
                    grasps, scores, timings["planning"], visual_mesh, tsdf_real_generate = grasp_plan_fn(state, intrinsic.copy(), extrinsic.copy(), rgb_imgs.copy(), depth_imgs.copy(), scene_mesh)
                else:
                ...
                if model_type == 'former3d':
                    grasps, scores, timings["planning"], tsdf_real_generate = grasp_plan_fn(state, intrinsic.copy(), extrinsic.copy(), rgb_imgs.copy(), depth_imgs.copy())
                else:
    ...
    return success_rate, declutter_rate, np.mean(planning_times), np.mean(total_times)
