...
from gd.utils.implicit import as_mesh

# import matplotlib
# matplotlib.use('Agg')  # 远程todesk的时候需要增加这行命令，加了之后plt.show不能用
...
def plot_voxel_grid(voxel, size=5):
    cube = (voxel>0)
    fcolors = np.empty(cube.shape, dtype=object)
    fcolors[cube] = 'sandybrown'
    # fcolors[cube] = 'mediumslateblue'
    fig = plt.figure(figsize=(size, size))
    ax = fig.add_subplot(projection='3d')
    # ax.voxels(cube, facecolors=fcolors, edgecolor='k')
    ax.voxels(cube, facecolors=fcolors, edgecolor='dimgray', linewidth=0.3)
    plt.axis("off")
    plt.tight_layout()
