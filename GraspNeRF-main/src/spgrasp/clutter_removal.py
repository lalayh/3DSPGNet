...
def run(
    ...
    for _ in tqdm.tqdm(range(num_rounds), disable=silence):
        ...
        while sim.num_objects > 0 and consecutive_failures < MAX_CONSECUTIVE_FAILURES:
            ...
            if resolution != 40:
                extra_tsdf, _, _, _, _, _, _ = sim.acquire_tsdf(n=n, N=N, resolution=resolution)
            ...
            if visualize:
                ...
                # image1 = Image.fromarray((depth_imgs[0] * 1000.0).round().astype(np.uint16))
